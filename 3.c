...
int main(void) {
	...
	uint8**   ppOutputInverseImageBuffer = 0;
	double**  transInputImageBuffer = 0;
	double**  realBuffer=0;
	double**  imaginaryBuffer=0;
	double** inverseBuffer=0;
	...
	inverseBuffer = (double**)malloc(sizeof(double*) * 256);
	for (i = 0; i<256; i++) {
		inverseBuffer[i] = (double*)malloc(sizeof(double) * 256);
	}
	ppOutputInverseImageBuffer = (unsigned char**)malloc(sizeof(unsigned char*) * 256);
	for (i = 0; i<256; i++) {
		ppOutputInverseImageBuffer[i] = (unsigned char*)malloc(sizeof(unsigned char) * 256);
	}
		...
		for (j = 0; j < 256; j++) {
			...
			for (s = 0; s < 256; s++) {
				...
				//	realBuffer[i][j] += (1.0 / (256.0 * 256.0) * pow(-1.0,(double)s+t) * ppInputImageBuffer[s][t] * (cos(2.0 * PI * (s*i + t*j) / 256.0)));
					...
					realBuffer[i][j] += ppInputImageBuffer[s][t] * (cos(2.0 * PI * (s*i + t*j) / 256.0));
					imaginaryBuffer[i][j] += ppInputImageBuffer[s][t] * (sin(2.0 * PI * (s*i + t*j) / 256.0));
				}
			}
			realBuffer[i][j] /= 65536;
			imaginaryBuffer[i][j] /= 65536;
		}
	}
	printf("DONE\n");
	for (i = 0; i < 256; i++) {
      for (j = 0; j < 256; j++) {
         ...
         for (s = 0; s < 256; s++) {
            for (t = 0; t < 256; t++) {
             //  inverseBuffer[i][j] += (i + j) % 2 == 0 ? (realBuffer[s][t] * cos(2.0 * PI * (s*i + t*j) / 256.0) - imaginaryBuffer[s][t] * sin(2.0 * PI * (s*i + t*j) / 256.0)) :
              //    (imaginaryBuffer[s][t] * sin(2.0 * PI * (s*i + t*j) / 256.0) - realBuffer[s][t] * cos(2.0 * PI * (s*i + t*j) / 256.0));
			   inverseBuffer[i][j] +=(realBuffer[s][t] * cos(2.0 * PI * (s*i + t*j) / 256.0) + imaginaryBuffer[s][t] * sin(2.0 * PI * (s*i + t*j) / 256.0));
            }
         }
         ppOutputInverseImageBuffer[i][j] = (uint8)inverseBuffer[i][j];
      }
...
/*
	...
	fpOutputImage = fopen("InverseDFT.img", "wb");

	for (i = 0; i<256; i++) {
		fwrite(ppOutputInverseImageBuffer[i], sizeof(unsigned char), 256, fpOutputImage);
	}
	printf("º¯È¯");
